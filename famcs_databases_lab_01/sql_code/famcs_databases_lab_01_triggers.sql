...
BEGIN
    SELECT COUNT(*) into number_of_rents FROM rent_contracts
        WHERE car_id = :new.car_id AND ((:new.rent_starting_date BETWEEN rent_starting_date AND rent_ending_date)
            ...
            OR (rent_starting_date BETWEEN :new.rent_starting_date AND :new.rent_ending_date)
...
BEGIN
    SELECT COUNT(*) into number_of_cars FROM rent_contracts
        WHERE renter_id = :new.renter_id AND ((:new.rent_starting_date BETWEEN rent_starting_date AND rent_ending_date)
            ...
            OR (rent_ending_date = :new.rent_starting_date));
    IF number_of_cars >= 2
    THEN RAISE_APPLICATION_ERROR(-20002, 'В аренде у одного клиента не может находиться более 2-х автомобилей одновременно !');
    END IF;
END;
/
...
BEGIN
  ...
  THEN RAISE_APPLICATION_ERROR(-20003, 'Окончательная стоимость аренды составляет менее 100 рублей !');
  END IF;
END;
